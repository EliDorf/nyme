...
export default function DomainFinder() {
  ...
  const handleSearch = async () => {
    ...
    trackDomainSearchInitiated(searchTerm, TLDs);
  ...
  const checkDomains = async (searchInput: string | string[], checkSuggestions: boolean) => {
    ...
    try {
      ...
      if (validDomains.length > 0) {
        ...
        setError(errorMsg);
        trackDomainSearchError('NO_RESULTS', errorMsg);
      }
    } catch (err) {
                  ...
                  <div className="flex flex-nowrap gap-2 pb-2 md:flex-wrap md:pb-0">
                      {suggestions.map((suggestion) => (
                        <Button
                          ...
                          onClick={() => {
                            trackDomainSuggestionClicked(suggestion, suggestions.indexOf(suggestion) + 1);
                              ...
                              <div key={domain.domain} className="flex items-center justify-between p-2 hover:bg-muted/50">
                                ...
                                <div className="flex items-center gap-4">
                                  <Button 
                                    ...
                                    size="sm"
                                onClick={() => {
                                  trackAddToCart(domain.domain, 9.99); // Using default price since actual price isn't available
