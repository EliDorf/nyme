...
interface DomainFinderProps {
    groqSuggestions?: string[];
}

export function DomainFinder({ groqSuggestions = [] }: DomainFinderProps) {
    ...
    const placeholderDomains: Domain[] = [
        ...
        { domain: 'Namsye.ia', available: true },
        { domain: 'Names.com', available: false },
        { domain: 'Nyme.co', available: false },
    ];

    useEffect(() => {
        if (groqSuggestions.length > 0) {
            checkDomains(groqSuggestions);
        }
    }, [groqSuggestions, selectedTLD]);
    ...
    const checkDomains = async (names: string[]) => {
        ...
        try {
            const requests = names.map(name => 
                axios.get(`/api/domains?name=${name}${selectedTLD}`)
            );
            const responses = await Promise.all(requests);
            const newDomains = responses.map((response, index) => ({
                domain: `${names[index]}${selectedTLD}`,
