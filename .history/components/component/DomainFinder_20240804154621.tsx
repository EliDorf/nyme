...
export function DomainFinder({ suggestions }: DomainFinderProps) {
    ...
    useEffect(() => {
        if (suggestions.length > 0) {
            checkDomains(suggestions);
        }
    }, [suggestions]);

    const checkDomains = async (names: string[]) => {
        setIsLoading(true);
        setError(null);
        try {
            const requests = names.flatMap(name => 
                TLDs.map(tld => 
                    axios.get(`/api/domains?name=${encodeURIComponent(name + tld)}`)
                )
            );
            const responses = await Promise.all(requests);
            const newDomains = responses.map((response, index) => {
                const fullDomain = `${names[Math.floor(index / TLDs.length)]}${TLDs[index % TLDs.length]}`;
    ...
    const isAvailable = (status: string) => {
        return status === "inactive" || status === "undelegated" || status === "unknown";
    };
    ...
    const unavailableDomains = displayDomains.filter(d => !isAvailable(d.status.status)).slice(0, 10);
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="py-2 px-4 bg-gray-100 dark:bg-gray-800">Not Available ({unavailableDomains.length} domains)</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
