    ...
    try {
      ...
      const requests = allDomains.flatMap(domain =>
        TLDs.map(tld => {
          ...
          return axios.get(`/api/domains?name=${encodeURIComponent(fullDomain)}`);
        })
      );
      ...
      const newDomains = responses.map((response, index) => {
        ...
        let status = response.data.status?.[0];
        if (!status || !status.domain) {
          console.log('Invalid status for domain:', domain, 'Creating default status');
          status = {
            domain: domain,
            zone: domain.split('.').pop() || '',
            status: 'unknown',
            summary: 'unknown'
          };
        }
        
        console.log('Processing domain:', domain, 'status:', status);
        return {
          domain,
          status
        };
      ...
      console.log('Setting domains:', newDomains);
      if (newDomains.length > 0) {
        setDomains(newDomains);
        setHasSearched(true);
        setDomainsChecked(true);
      } else {
        console.error('No valid domains found in response');
