...
export function DomainFinder({ inputDomain, suggestions }: DomainFinderProps) {
    ...
    const checkDomains = async (domain: string) => {
        setIsLoading(true);
        setError(null);
        try {
            const requests = TLDs.map(tld => 
                axios.get(`/api/domains?name=${encodeURIComponent(domain + tld)}`)
            );
            const responses = await Promise.all(requests);
            const newDomains = responses.map((response, index) => {
                const fullDomain = `${domain}${TLDs[index]}`;
        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="border rounded-lg overflow-auto shadow-md">
                <Table>
                    ...
                    <TableBody>
                        {Array.from({ length: Math.max(availableDomains.length, Math.min(unavailableDomains.length, 30)) }).map((_, index) => (
                            <TableRow key={index}>
