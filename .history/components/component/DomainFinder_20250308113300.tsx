...
export default function DomainFinder() {
  ...
  const handleSearch = async () => {
    ...
    trackDomainSearchInitiated(searchTerm, selectedTLDs, userEmail);
  ...
  useEffect(() => {
    if (hasSearched && suggestions.length > 0 && lastSearchTerm) {
      ...
      const newSuggestions = suggestions.filter(suggestion => 
        !selectedTLDs.some(tld => existingDomains.has((suggestion + tld).toLowerCase()))
      );
  ...
  const checkDomains = async (searchInput: string | string[], checkSuggestions: boolean) => {
    ...
    try {
      ...
      const domainPromises = domainsToCheck.flatMap(domain =>
        selectedTLDs.map(async (tld): Promise<Domain | null> => {
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                      <Button
                        ...
                        className="rounded-md px-3 flex items-center justify-center w-full sm:w-auto"
                      ...
                      >
                        ...
                        <span>Synonym Names</span>
                      </Button>
                    </div>
                    <div className="text-xs text-muted-foreground text-center sm:text-right mt-1 sm:mt-0">
